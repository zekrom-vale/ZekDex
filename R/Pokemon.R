...
HTMLS = map(URLS, function(URL){
	Sys.sleep(sample(1:10, 1))
	HTML = rvest::read_html(URL)
	write_rds(as.character(HTML), paste0("E:\\html\\", str_extract(URL, "wiki/(.*)_\\(Pok\\%C3\\%A9mon\\)", group = 1), ".rds"))
})

zek = read_rds("E:\\html\\Zekrom.rds")
...
check_h <- function(x, tag) {
	if(inherits(x, "xml_node")) {
		rvest::html_name(x) == tag
	} else if (is.list(x)) {
		any(unlist(lapply(x, check_h, tag)))
	} else {
		FALSE
	}
}
