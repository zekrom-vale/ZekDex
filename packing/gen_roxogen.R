...
convert_yaml_to_roxygen = function(yaml_data, dataset_name) {
	...
	yaml_data =  map(
		yaml_data,
		~{
			if(!is.list(.x)) str_replace_all(.x, "\n", "\n#' ")
			else .x
		}
	)
	...
	yaml_data$examples = paste("#'", yaml_data$examples)

	# Use glue to fill in the template
	roxygen = glue_data(yaml_data, roxygen_template)
...
writeLines(c("# Generated by packing/gen_roxogen.R", "# Do not edit by hand, instead edit packing/data.yaml", roxygen_list), "R/data.R")
