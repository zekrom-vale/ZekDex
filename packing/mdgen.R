...
#' This function converts Roxygen2 dataset documentation in an R file to a Markdown format.
#' It reads the text from an input R file, performs the necessary conversions,
#' and writes the converted text to Markdown files.
#'
#' @param input_file A character string specifying the path to the input R file.
#' @param gl A character string specifying the path to the output Markdown file.
#'
...
#'
#' @return NULL
migrate_text = function(input_file, gl = "wiki/data/{quoted_string}.md"){
	# Read the input file
	input_text = read_lines(input_file)
	...
	for (section in sections) {
		...
		# Add the quoted string to the top of the section output with ### added before it
		# section_output = c(section_output, glue("# {quoted_string}"))
		...
		# Write the section output to a Markdown file
		write_lines(section_output, glue(gl))
	}
}

migrate_text("R/data.R")
